...
その入力フォルダにファイルを置いた場合、1 分ごとのトリガーで `processInputFolder()` が実行され、以下の判定順で処理されます。

## 全体フロー

1. 入力フォルダ内の全ファイルを順番に確認
2. `clickpostReport` 対象か判定
3. `RPP運用CSV` の対象か判定
4. それ以外は通常の子 GAS 振り分け処理
5. 最後に `RPP運用CSV` をペア単位でまとめて処理
...
入力フォルダにファイルを置いたときの挙動は、要約すると以下です。
