...
RPPトラック系と広告表示ファイルが同時にある場合、RPPトラック系がすべて成功した後、広告表示の処理前に約3分待機します。待機中の広告表示ファイルと楽天サーチファイルは `input` に残り、1分ごとの次回以降のトリガーで3分経過を確認してから処理されます。
...
RPPトラック系の後続として広告表示を処理する場合は、RPPトラック系の成功後すぐには実行せず、約3分経過してから処理します。`Utilities.sleep` で実行を止めるのではなく、待機開始時刻を保存し、1分ごとのトリガーで経過時間を判定します。
